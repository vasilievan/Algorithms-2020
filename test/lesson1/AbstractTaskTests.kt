...
abstract class AbstractTaskTests : AbstractFileTests() {
    ...
    protected fun sortAddresses(sortAddresses: (String, String) -> Unit) {
        ...
        try {
            sortAddresses("input/addr_in_vasiliev.txt", "temp.txt")
            assertFileContent("temp.txt", File("input/addr_out_vasiliev.txt").readLines())
        } finally {
            File("temp.txt").delete()
        }
    }
    ...
    protected fun sortTemperatures(sortTemperatures: (String, String) -> Unit) {
        ...
        try {
            sortTemperatures("input/temp_in_vasiliev.txt", "temp.txt")
            assertFileContent(
                "temp.txt",
                """
                    -100.0
